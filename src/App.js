    ...
    async function fetchData() {
      try {
        ...
        const res = await fetch(
          `https://www.omdbapi.com/?apikey=918e76cd&s=${query}`,
          { signal: controller.signal }
        );

        if (!res.ok) throw new Error("錯誤訊息");

        const data = await res.json();
        if (data.Response === "False") throw new Error("找不到影片...");
...
function MovieDetails({ selectId, onCloseMovie, onAddWatched, watched }) {
  ...
  useEffect(() => {
    ...
    async function getMovieDetails() {
      try {
        const res = await fetch(
          `https://www.omdbapi.com/?apikey=918e76cd&i=${selectId}`
        );
        const data = await res.json();
        setMovie(data);
      } catch (err) {
